...
var CACHE_VERSION = 4;
var CURRENT_CACHES = {
  files: 'blaya__FILES_CACHE_V' + CACHE_VERSION,
  playlistTracks: 'blaya__PLAYLIST_TRACKS_CACHE_V' + CACHE_VERSION,
};
...
//   event.respondWith(
//     caches.open(CURRENT_CACHES.files).then(function(cache) {
//       return cache.match(event.request).then(fileCache(cache, event)).catch(errorHandler);
//     })
//   );
// });
...
self.addEventListener('fetch', function(event) {
  event.respondWith(
    caches.open(CURRENT_CACHES.playlistTracks).then(function(cache) {
      return cache.match(event.request).then(trackCache(cache, event)).catch(errorHandler);
    })
  );
});

function trackCache(cache, event) {
  return response => {
    if (response) {
      return response
    }
    return fetch(event.request.clone()).then(function(response) {
      if (
        response.status < 400
        && event.request.url.includes('?cacheme=true')
        && response.headers.has('content-type')
        && (
          response.headers.get('content-type').match(/^audio/i)
        )
      ) {
        console.log(`@FILTER Caching response from cacheme true:`, response)
        // TODO: this doesn't work locally because the failed responses (from aws cli) aren't content-type audio...
        cache.put(event.request, response.clone());
      } else {
        console.log(`Not caching..`, event.request)
      }
...
function fileCache(cache, event) {
  return response => {
    if (response) {
    ...
    return fetch(event.request.clone()).then(function(response) {
      ...
      if (
        ...
        && response.headers.has('content-type')
        && (
          response.headers.get('content-type').match(/^application\/javascript/i)
