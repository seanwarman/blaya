...
exports.copySendFile = (req, res) => {
	...
	access(__dirname + filePath, constants.F_OK, (err) => {
		...
		const cp = spawn('aws', ['s3', 'cp', 's3://everest-files/complete/' + filePath, __dirname + '/../complete/' + filePath])
		cp.on('error', error => {
			console.log(error)
			res.send(error)
		})
		cp.on('close', () => {
			res.sendFile('/complete/' + filePath, { root: __dirname + '/..' })
		})
	})
}
...
exports.streamFile = (req, res) => {
	const filePath = req.params[0]
	const cp = spawn('aws', ['s3', 'cp', 's3://everest-files/complete/' + filePath, '-'])
	cp.stdout.pipe(res)
}
